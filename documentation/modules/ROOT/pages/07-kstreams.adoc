...
One of the important thing of Kafka Streams application is that it doesn't run inside a broker, but it runs in a spearate JVM instance, maybe in the same cluster, or maybe in a different cluster but it is a different process.
...
This is what makes Kafka Streams applications fault-tolerancea and scalable.

[#kstreamsconcepts]
== Kafka Sterams Concepts

Stream Processor:: A Stream processor represents an operation to execute to a stream, some examples of built-in operations can be filter, map, join or aggregate. 
Usually, a Kafka Stream application is created of one or more operations.

Table:: A table is a collection of key-value pairs, that represents the last value for the same record key.
The big difference is that a stream contains the log of all events (ie an insertation of a song, and update of a song, ...) and a table contains the last value (ie the last update done in a song).
There are two kind of tables, a _KTable_ which represents a table from a partition, and a _GlobalKTable_ which agrgegates the content from all partitions of a given topic.
...
Windowing:: Group records that have the same key by a window time. For example you can group which events has occurred between a period of time.
