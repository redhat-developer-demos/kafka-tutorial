...
Check that everything has been installed by running the follwing commands:
...
In this case, you are going to deploy a Kafka cluster with 3 instances and ephimeral storage.
...
Run the follwoing command to deploy Kafka:
...
To wait until Kafka cluster is up and running, you can run the follwoing comamnd:
...
In this specific case `kafka-tutorial-song-app` port is `31834` and `kafka-tutorial-song-indexer-app` is `30581` but this is going to change everytime you deploy these services.

Then you need the IP to connect.
This IP is the minikube IP and it is know by running the following command:
