...
Switch back to _terminal 1_ where you are running `kafkacat` as consumer.
...
And start again with the follwoing command:
...
By default, Kafka retains data in a topic for 168 hours (7 days).
But retention can be changed, let's change it to 1 minute.

You can also use `kafka-topic.sh` tool to change retention time of a topic.
...
Stop `kafkacat` process by typing kbd:[Ctrl + C] and wait for ~10 seconds (ie `sleep 15`).

Run `kafkacat` in consumer mode again:
...
The reason is that the messages are expired and deleted from the topic, but the offset has its own retention period.
